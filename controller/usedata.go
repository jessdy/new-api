...
func GetQuotaDatesByAgent(c *gin.Context) {
	startTimestamp, _ := strconv.ParseInt(c.Query("start_timestamp"), 10, 64)
	endTimestamp, _ := strconv.ParseInt(c.Query("end_timestamp"), 10, 64)
	dates, err := model.GetQuotaDataGroupByAgent(startTimestamp, endTimestamp)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func GetAgentUserQuotaDates(c *gin.Context) {
	...
	dates, err := model.GetQuotaDataByAgentId(agentId, startTimestamp, endTimestamp)
	if err != nil {
		common.ApiError(c, err)
		return
	}
